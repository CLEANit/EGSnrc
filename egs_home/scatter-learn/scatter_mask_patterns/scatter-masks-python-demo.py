...
for cix in range(len(color_func_list)):
    ...
    color_func = binary
    binN = np.random.randint(0,11)*np.random.randint(0,2**63)#2**64 - 2**32 + 1# -1 #- 1 # -1 
    ...
    for xi in [xrm[0],xrm[1]-1]:#range(-4,4):
        ...
        for yi in range(yrm[0],yrm[1]):
            ...
            for zi in range(zrm[0],zrm[1]):
                ...
                ax.plot_surface(xr[0]*onez ,np.vstack((yr,yr)) ,np.dstack((zr,zr))[0] ,color=color_list[int(color_func(xi,yi,zi,binN))])
                ax.plot_surface(xr[1]*onez ,np.vstack((yr,yr)) ,np.dstack((zr,zr))[0] ,color=color_list[int(color_func(xi,yi,zi,binN))])
                ax.plot_surface(np.vstack((xr,xr)) ,yr[0]*onez ,np.dstack((zr,zr))[0] ,color=color_list[int(color_func(xi,yi,zi,binN))])
                ax.plot_surface(np.vstack((xr,xr)) ,yr[1]*onez ,np.dstack((zr,zr))[0] ,color=color_list[int(color_func(xi,yi,zi,binN))])
                ax.plot_surface(np.vstack((xr,xr)) ,np.dstack((yr,yr))[0] ,zr[0]*onez ,color=color_list[int(color_func(xi,yi,zi,binN))])
                ax.plot_surface(np.vstack((xr,xr)) ,np.dstack((yr,yr))[0] ,zr[1]*onez ,color=color_list[int(color_func(xi,yi,zi,binN))])
